...
#*****************************************
(bfw_mod <- nls(hnl2_Xs_bfw~a*f_bfw^b, bfw, start=list(a=.0362, b=.74)))
(summary(bfw_mod))
plot(with(bfw,hnl2_Xs_bfw~f_bfw),pch=21,col='black', cex=1, bg='white', 
     xlab = (""), ylab = (""), xlim=range(bfw$f_bfw),ylim=c(0,42))
abline(0,1, lty=2, col='grey60')#1:1 line
par(new=T)
curve(4.33198*x^0.49750, xlim=range(bfw$f_bfw), ylim=c(0,42),
...
(TSS <- sum((bfw$hnl2_Xs_bfw - mean(bfw$hnl2_Xs_bfw))^2))  # Total sum of squares
...
#************************************
plot(with(vw,hnl2_Xs_vw~opt_vw), xlim=range(vw$opt_vw), ylim=c(0,424),
     xlab='Optical valley width (m)',ylab='Model valley width (m)')
abline(0,1, lty=2,col="grey60")#1:1 line
(vw_mod <- lm(hnl2_Xs_vw~opt_vw,data=vw))
summary(vw_mod)
clip(min(x=min(vw$opt_vw)), max(x=max(vw$opt_vw)), min(y=min(vw$hnl2_Xs_vw)), max(y=max(vw$hnl2_Xs_vw)))
...
## Valley Width Index; Constraint Plot; hnl2 Xs vs optical opt
#************************
plot(with(vw,d_vwi~f_vwi), col='black', xlim=range(vw$f_vwi), ylim=c(1,18.7),
     xlab="Optical field VWI",ylab="Model VWI")
abline(0,1, lty=2, col='grey60')#1:1 line
(vwi_mod <- lm(d_vwi~f_vwi,data=vw))
summary(vwi_mod)
summary(vwi_mod)$r.squared
clip(min(x=min(vw$f_vwi)), max(x=max(vw$f_vwi)), min(y=min(vw$d_vwi) ), max(y=max(vw$d_vwi)))
...
plot(with(bfd,hnl2_bfd~f_bfd), col='black', xlim=range(bfd$f_bfd), ylim=c(0.05,.81),
     xlab="",ylab="")
abline(0,1, lty=2, col='grey60')#1:1 line
(bfd_mod <-nls(hnl2_bfd~a*f_bfd^b, bfd, start=list(a=.0362, b=.74)))
(summary(bfd_mod))
par(new=T)
curve(0.75909*x^0.27979, xlim=range(bfd$f_bfd), ylim=c(0.05,.81),
...
plot(with(d50,hnl2_d50~f_d50), col='black', xlim=range(5,165), ylim=c(5,165),
...
rm(anad)
plot(with(q_w,WIDTH_M~MEANANNCMS), col='grey', xlim=c(0,12), ylim=c(0,30),
     xlab="DEM Q (cms)",ylab="DEM BFW (cms)")
#abline(0,1, lty=2, col='grey60')#1:1 line
par(new=T)# redd dots are IP sites
plot(with(q_w1,WIDTH_M~MEANANNCMS), col='red', pch=16, cex=.75,xlim=c(0,12), ylim=c(0,30),
     xlab="DEM Q (cms)",ylab="DEM BFW (cms)")
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted",
     lwd = par("lwd"), equilogs = TRUE)
