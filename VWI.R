...
library(MASS)
with(bfw,
     {boxcox(hnl2_Xs_bfw~f_bfw, lambda=seq(1,2,0.01))
     })

with(bfw,#a power transformation of 1.6, as indicated from boxcox test for linear model
     {plot(hnl2_Xs_bfw^1.6~f_bfw)
      abline(lm(hnl2_Xs_bfw^1.6~f_bfw))
      summary(lm(hnl2_Xs_bfw^1.6~f_bfw,bfw))
      par(mfrow=c(2,2))
      plot(lm(hnl2_Xs_bfw^1.6~f_bfw,bfw))
      par(op)
       })
...
with(bfw,
     {(bfw_mod <- nls(hnl2_Xs_bfw~a*f_bfw^b, start=list(a=.0362, b=.74)))
       ...
       curve(4.33198*x^0.49750, xlim=range(f_bfw), ylim=c(0,42),
             col="darkgreen",lwd= 2,xlab="Field bankfull width (m)", ylab="Model bankfull width (m)")
       ...
       (cd <- round(1 - (RSS.p/TSS),3)) # Coefficient of determination;R-squared
       (cf <- round(coef(bfw_mod), 2)) # Model coefficient (a,b)
       ## sign check to avoid having plus followed by minus for negative coefficients
       eq <- paste0("Model BFW (m) = ", cf[1],'* Field BFW (m)^', cf[2])
...
## VW Data; opt_vw was optical estimate from DEM in ArcGIS using the measure tool
...
library(MASS)
with(vw,
     {boxcox(hnl2_Xs_vw~opt_vw, lambda=seq(0,2,0.01))
...
with(vw,
     ...
     xlab="Optical-field valley width index",ylab="Model valley width index")
      abline(0,1, lty=2, col='grey60')#1:1 line
...
with(d50,
     ...
     xlab="Field D50 (mm)",ylab="DEM D50 (mm)")
      ...
      grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted",
           lwd = par("lwd"), equilogs = TRUE)
      summary(lm(hnl2_d50~f_d50,d50))
      abline(lm(hnl2_d50~f_d50,d50))
})
